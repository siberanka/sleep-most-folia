package me.mrgeneralq.sleepmost.core.runnables;
...
import me.mrgeneralq.sleepmost.core.statics.Time;
...
public class Heartbeat {
    ...
    public Heartbeat(ISleepService sleepService, ISleepMostWorldService sleepMostWorldService,
            IInsomniaService insomniaService, IFlagsRepository flagsRepository) {
    ...
    private void checkUnfreezeRequired(World world) {
        ...
        if (sleepMostWorld.getFrozenUntil().getTimeInMillis() > Calendar.getInstance().getTimeInMillis()
                && !sleepMostWorld.isTimeCycleAnimationIsRunning())
            return;
